...
static void switchStatement() {
	...
	int nextJump = -1;
	int* exitJumps = malloc(sizeof(int));
	...
	while (match(TOKEN_CASE)) {
		...
		if (count + 1 > capacity) {
			...
			exitJumps = GROW_ARRAY(int, exitJumps, oldCapacity, capacity);
		}

		exitJumps[count] = exitJump;
		count++;
	}
	...
	FREE_ARRAY(int, exitJumps, capacity);
}
...
static void synchronize() {
	...
	while (parser.current.type != TOKEN_EOF) {
		if (parser.current.type == TOKEN_SEMICOLON) break;
